...
def dump_graph_bn(bbs, simple, notes, colored_links):
    g = FlowGraph()
    ...
    for b in bbs.values():
        ...
        body = []
        for n in b.nested:
            body.append(f"{hex(n.addr)} {n.text}")
        node.lines = body
    ...
    for b in bbs.values():
        for s in b.succ:
            nodes[b.addr].add_outgoing_edge(BranchType.UnconditionalBranch, nodes[s])

    show_graph_report("Execution trace", g)

def do_exec_cfg(bv, _):
    path = interaction.get_directory_name_input("Directory containing *.trace files")
    traces = []

    for fname in os.listdir(path):
        fname = fname.decode('ascii')
        if not fname.endswith(".trace"):
            continue

        traces.append(path.decode('ascii') + "/" + fname)
    ...
    dump_graph_bn(simple_bbs, False, {}, colored_links)
